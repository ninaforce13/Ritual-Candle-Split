...
func _use_in_world(_node, _context, _arg):
	...
	if not start_ritual_type_candle():
...
func start_ritual_type_candle()->bool:
	if SaveState.other_data.has("bootleg_ritual_type_candle") or SaveState.other_data.has("bootleg_ritual_candle"):
		return false
	SaveState.other_data.bootleg_ritual_type_candle = {}
	return true
